...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      attrPos(-1), attrNor(-1), attrCol(-1), attrUV(-1), attrAnim(-1), attrCosPower(-1),
      attrPosT(-1), attrNorT(-1), attrColT(-1), attrUVT(-1), attrAnimT(-1), attrCosPowerT(-1),
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifColor(-1),
      unifSampler(-1), unifTime(-1), unifCam(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrCosPower = context->glGetAttribLocation(prog, "vs_CosPower");

    attrPosT = context->glGetAttribLocation(prog, "vs_PosT");
    attrNorT = context->glGetAttribLocation(prog, "vs_NorT");
    attrColT = context->glGetAttribLocation(prog, "vs_ColT");
    attrUVT = context->glGetAttribLocation(prog, "vs_UVT");
    attrAnimT = context->glGetAttribLocation(prog, "vs_AnimT");
    attrCosPowerT = context->glGetAttribLocation(prog, "vs_CosPowerT");
...
void ShaderProgram::setPlayerPos(const glm::vec3 &pos)
{
    useMe();
...
void ShaderProgram::setTexture(int i)
{
    ...
    if(unifSampler != -1)
    {
        context->glUniform1i(unifSampler, /*GL_TEXTURE*/i);
    }
}
...
void ShaderProgram::draw_chunk(Drawable &c) {
    useMe();

    if (attrPos != -1 && c.bind_attr()) {
        context->glEnableVertexAttribArray(attrPos);
        context->glVertexAttribPointer(attrPos, 4, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, pos)));
    }

    if (attrNor != -1 && c.bind_attr()) {
        context->glEnableVertexAttribArray(attrNor);
        context->glVertexAttribPointer(attrNor, 4, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, norm)));
    }

    if (attrCol != -1 && c.bind_attr()) {
        context->glEnableVertexAttribArray(attrCol);
        context->glVertexAttribPointer(attrCol, 4, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, col)));
    }

    if (attrUV != -1 && c.bind_attr()) {
        context->glEnableVertexAttribArray(attrUV);
        context->glVertexAttribPointer(attrUV, 2, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, uv)));
    }

    if (attrAnim != -1 && c.bind_attr()) {
        context->glEnableVertexAttribArray(attrAnim);
        context->glVertexAttribPointer(attrAnim, 2, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, anim)));
    }

    if (attrCosPower != -1 && c.bind_attr()) {
        context->glEnableVertexAttribArray(attrCosPower);
        context->glVertexAttribPointer(attrCosPower, 1, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, cos_pow)));
    }
    ...
    if (attrCosPower != -1) context->glDisableVertexAttribArray(attrCosPower);

    context->printGLErrorLog();
}

void ShaderProgram::draw_chunkT(Drawable &c) {
    useMe();

    if (attrPos != -1 && c.bind_attrT()) {
        context->glEnableVertexAttribArray(attrPos);
        context->glVertexAttribPointer(attrPos, 4, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, pos)));
    }

    if (attrNor != -1 && c.bind_attrT()) {
        context->glEnableVertexAttribArray(attrNor);
        context->glVertexAttribPointer(attrNor, 4, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, norm)));
    }

    if (attrCol != -1 && c.bind_attrT()) {
        context->glEnableVertexAttribArray(attrCol);
        context->glVertexAttribPointer(attrCol, 4, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, col)));
    }

    if (attrUV != -1 && c.bind_attrT()) {
        context->glEnableVertexAttribArray(attrUV);
        context->glVertexAttribPointer(attrUV, 2, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, uv)));
    }

    if (attrAnim != -1 && c.bind_attrT()) {
        context->glEnableVertexAttribArray(attrAnim);
        context->glVertexAttribPointer(attrAnim, 2, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, anim)));
    }

    if (attrCosPower != -1 && c.bind_attrT()) {
        context->glEnableVertexAttribArray(attrCosPower);
        context->glVertexAttribPointer(attrCosPower, 1, GL_FLOAT, false, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, cos_pow)));
    }
    ...
    // This invokes the shader program, which accesses the vertex buffers.
    c.bindIdxT();
    context->glDrawElements(c.drawMode(), c.elemCountT(), GL_UNSIGNED_INT, 0);
