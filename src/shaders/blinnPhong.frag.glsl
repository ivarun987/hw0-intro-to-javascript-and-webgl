...
void main()
{
    ...
    vec4 viewVec = normalize(fs_Pos - fs_CameraPos);
    vec4 lightVec = normalize(fs_Pos - fs_LightVec);
    vec4 average = (viewVec + lightVec) / 2;


    float specularIntensity = max(pow(dot(average, fs_Nor), 10), 0);
    specularIntensity += lightIntensity;
    // Compute final shaded color
    out_Col = vec3(diffuseColor.rgb * specularIntensity);
